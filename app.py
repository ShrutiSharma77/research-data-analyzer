...
def process():
    ...
    if xlsx_file:
        ...
        for value in averages_flattened:
            average_values.append(f"{j}. {value}")
            j = j + 1
        ...
        return render_template('result.html', averages=average_values, x=x_new, y_new=y_new.tolist(), normalized_y=y.tolist(), range_value=range_value, max_value=max_value, total_grains=total_grains)
